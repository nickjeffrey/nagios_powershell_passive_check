...
function Get-TSM-Client-Backup-Age {
   ...
   if (-Not(Test-Path $fileToCheck -PathType leaf)) {                #run this section if $fileToCheck does NOT exist
      $exit_code = $WARN 								 #0=ok 1=warn 2=critical 3=unknown
      $plugin_output = "$service WARN - process $processtoCheck is running, but the $fileToCheck backup logfile cannot be found."
   }
   if (Test-Path $fileToCheck -PathType leaf) {       #run this section if $fileToCheck exists
